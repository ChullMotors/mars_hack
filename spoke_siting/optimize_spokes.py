...
def select_spokes(cand, mean, std, heat, n_spokes, range_km, max_area=MAX_PV_AREA_M2, hub_import=0.0, hubs=None):
    ...
    Each candidate's PV array is sized so its 95%-LCB capacity meets 400 kWh/sol. Capacity
    at area A is  A * k - heat  (PV output scales with area, the heating load does not), so
        required_area = BASE * (DEMAND - import + heat) / (LCB(capacity at BASE) + heat)
    where `heat` is the GP-predicted mean heating load (kWh/sol) at the site. Sites needing more than `max_area` are infeasible.
    `hub_import` (kWh/sol) is energy the hub pipes to the spoke every sol (H2 from hub
    electrolysis); a constant per-sol import is exactly a demand reduction, so the same GP
    serves every import scenario. Score = -panel cost - pipeline cost - crowding, so the optimiser trades
    ...
    with np.errstate(divide="ignore"):
        pv_at_base = lcb + heat                      # what the array alone delivers at BASE area
        req_area = np.where(pv_at_base > 1.0,
    ...
    for j in chosen:
        rows.append(dict(hub_id=int(cand["hub_id"][j]), lat=lat[j], lon=lon[j], elev_m=float(cand["elev"][j]),
                         ...
                         pv_area_m2=float(min(req_area[j], max_area)),
                         required_pv_area_m2=float(req_area[j]), hub_import_kwh_per_sol=hub_import))
    return rows
